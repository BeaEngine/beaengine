...
class TestSuite:
    ...
    def test(self):
        ...
        Buffer = bytes.fromhex('f30f38df00')
        ...
        assert_equal(myDisasm.infos.Instruction.Mnemonic, b'aesdec256kl ')
        ...
        assert_equal(myDisasm.repr(), 'aesdec256kl xmm0,  [rax]')

        # 66 0F 38 DC /r
        # AESdeclast xmm1, xmm2/m128

        Buffer = bytes.fromhex('660f38df6bA2')
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        assert_equal(hex(myDisasm.infos.Instruction.Opcode), '0xf38df')
        assert_equal(myDisasm.infos.Instruction.Mnemonic, b'aesdeclast ')
        assert_equal(myDisasm.repr(), 'aesdeclast xmm5, xmmword ptr [rbx-5Eh]')
        ...
        Buffer = bytes.fromhex('c4{:02x}{:02x}df6ba2'.format(myVEX.byte1(), myVEX.byte2()))
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        assert_equal(myDisasm.repr(), 'vaesdeclast xmm5, xmm0, xmmword ptr [rbx-5Eh]')
        ...
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        assert_equal(myDisasm.infos.Instruction.Mnemonic, b'vaesdeclast ')
