...
class TestSuite:
    def test(self):
        ...
        Buffer = bytes.fromhex('f30f38fbc0')
        myDisasm = Disasm(Buffer)
        length = myDisasm.read()
        assert_equal(length, len(Buffer))
        assert_equal(myDisasm.infos.Instruction.Opcode, 0xf38fb)
        assert_equal(myDisasm.infos.Instruction.Category, KL_INSTRUCTION)
        assert_equal(myDisasm.infos.Instruction.Mnemonic, b'encodekey256 ')
        ...
        assert_equal(myDisasm.infos.Operand6.Registers.xmm, REG3)
        ...
        assert_equal(myDisasm.infos.Operand7.Registers.xmm, REG4)
        ...
        assert_equal(myDisasm.infos.Operand8.Registers.xmm, REG5)
        ...
        assert_equal(myDisasm.infos.Operand9.Registers.xmm, REG6)
        assert_equal(myDisasm.infos.Operand9.OpSize, 128)
        assert_equal(myDisasm.infos.Operand9.AccessMode, WRITE)

        assert_equal(myDisasm.repr(), 'encodekey256 eax, eax')
