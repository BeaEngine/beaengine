...
class TestSuite:
    def test(self):
        ...
        assert_equal(myDisasm.infos.Instruction.Mnemonic, b'vpslld ')
        assert_equal(myDisasm.repr(), 'vpslld xmm25, xmm31, xmmword ptr [r14]')

        # EVEX.NDS.256.66.0F.WIG f2 /r
        # vpslld ymm1 {k1}{z}, ymm2, ymm3/m256
        Buffer = bytes.fromhex('62010520f20e')
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        assert_equal(myDisasm.infos.Reserved_.EVEX.P0, 0x1)
        assert_equal(myDisasm.infos.Reserved_.EVEX.P1, 0x5)
        ...
        assert_equal(hex(myDisasm.infos.Instruction.Opcode), '0xf2')
        assert_equal(myDisasm.infos.Instruction.Mnemonic, b'vpslld ')
        assert_equal(myDisasm.repr(), 'vpslld ymm25, ymm31, ymmword ptr [r14]')

        # EVEX.NDS.512.66.0F.WIG f2 /r
        # vpslld zmm1 {k1}{z}, zmm2, zmm3/m512
        Buffer = bytes.fromhex('62010540f20e')
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        assert_equal(myDisasm.infos.Reserved_.EVEX.P0, 0x1)
        assert_equal(myDisasm.infos.Reserved_.EVEX.P1, 0x5)
        ...
        assert_equal(hex(myDisasm.infos.Instruction.Opcode), '0xf2')
        assert_equal(myDisasm.infos.Instruction.Mnemonic, b'vpslld ')
        assert_equal(myDisasm.repr(), 'vpslld zmm25, zmm31, zmmword ptr [r14]')
