...
class TestSuite:
    def test(self):
        ...
        Buffer = 'c40201ab443322'.decode('hex')
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        assert_equal(myDisasm.instr.Instruction.Mnemonic, 'vfmsub213ss ')
        assert_equal(myDisasm.instr.repr, 'vfmsub213ss xmm8, xmm15, xmmword ptr [r11+r14+22h]')

        # VEX.NDS.256.66.0F38.W0 ab /r
        # vfmsub213ss ymm1, ymm2, ymm3/m256
        Buffer = 'c40205ab443322'.decode('hex')
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        assert_equal(myDisasm.instr.Instruction.Mnemonic, 'vfmsub213ss ')
        assert_equal(myDisasm.instr.repr, 'vfmsub213ss ymm8, ymm15, ymmword ptr [r11+r14+22h]')

        # EVEX.NDS.128.66.0F38.W0 ab /r
        # vfmsub213ss xmm1 {k1}{z}, xmm2, xmm3/m128
        Buffer = '62020506ab443322'.decode('hex')
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        ...
        assert_equal(hex(myDisasm.instr.Instruction.Opcode), '0xab')
        assert_equal(myDisasm.instr.Instruction.Mnemonic, 'vfmsub213ss ')
        assert_equal(myDisasm.instr.repr, 'vfmsub213ss xmm0, xmm15, xmmword ptr [rbx+rsi+22h]')

        # EVEX.NDS.256.66.0F38.W0 ab /r
        # vfmsub213ss ymm1 {k1}{z}, ymm2, ymm3/m256
        Buffer = '62020520ab443322'.decode('hex')
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        assert_equal(myDisasm.instr.Reserved_.EVEX.P0, 0x2)
        assert_equal(myDisasm.instr.Reserved_.EVEX.P1, 0x5)
        ...
        assert_equal(hex(myDisasm.instr.Instruction.Opcode), '0xab')
        assert_equal(myDisasm.instr.Instruction.Mnemonic, 'vfmsub213ss ')
        assert_equal(myDisasm.instr.repr, 'vfmsub213ss ymm0, ymm15, ymmword ptr [rbx+rsi+22h]')

        # EVEX.NDS.512.66.0F38.W0 ab /r
        # vfmsub213ss zmm1 {k1}{z}, zmm2, zmm3/m512
        Buffer = '62020540ab443322'.decode('hex')
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        assert_equal(myDisasm.instr.Reserved_.EVEX.P0, 0x2)
        assert_equal(myDisasm.instr.Reserved_.EVEX.P1, 0x5)
        ...
        assert_equal(hex(myDisasm.instr.Instruction.Opcode), '0xab')
        assert_equal(myDisasm.instr.Instruction.Mnemonic, 'vfmsub213ss ')
        assert_equal(myDisasm.instr.repr, 'vfmsub213ss zmm0, zmm15, zmmword ptr [rbx+rsi+22h]')
        ...
        Buffer = 'c4{:02x}{:02x}ab443322'.format(myVEX.byte1(), myVEX.byte2()).decode('hex')
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        assert_equal(myDisasm.instr.Instruction.Mnemonic, 'vfmsub213sd ')
        assert_equal(myDisasm.instr.repr, 'vfmsub213sd xmm8, xmm15, xmmword ptr [r11+r14+22h]')
