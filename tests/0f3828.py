...
class TestSuite:
    def test(self):
        ...
        myDisasm.read()
        assert_equal(hex(myDisasm.instr.Instruction.Opcode), '0xf3828')
        assert_equal(myDisasm.instr.Instruction.Mnemonic, 'pmuldq ')
        assert_equal(myDisasm.instr.repr, 'pmuldq xmm2, xmmword ptr [rax+44332211h]')

        # VEX.NDS.128.66.0F38.WIG 28 /r
        # vpmuldq xmm1, xmm2, xmm3/m128
        Buffer = 'c4020128443322'.decode('hex')
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        assert_equal(myDisasm.instr.Instruction.Mnemonic, 'vpmuldq ')
        assert_equal(myDisasm.instr.repr, 'vpmuldq xmm8, xmm15, xmmword ptr [r11+r14+22h]')

        # VEX.NDS.256.66.0F38.WIG 28 /r
        # vpmuldq ymm1, ymm2, ymm3/m256
        Buffer = 'c4020528443322'.decode('hex')
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        assert_equal(myDisasm.instr.Instruction.Mnemonic, 'vpmuldq ')
        assert_equal(myDisasm.instr.repr, 'vpmuldq ymm8, ymm15, ymmword ptr [r11+r14+22h]')

        # EVEX.NDS.128.66.0F38.WIG 28 /r
        # vpmuldq xmm1 {k1}{z}, xmm2, xmm3/m128
        Buffer = '6202050628443322'.decode('hex')
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        ...
        assert_equal(myDisasm.instr.Instruction.Mnemonic, 'vpmuldq ')
        assert_equal(myDisasm.instr.repr, 'vpmuldq xmm0, xmm15, xmmword ptr [rbx+rsi+22h]')

        # EVEX.NDS.256.66.0F38.WIG 28 /r
        # vpmuldq ymm1 {k1}{z}, ymm2, ymm3/m256
        Buffer = '6202052028443322'.decode('hex')
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        assert_equal(myDisasm.instr.Reserved_.EVEX.P0, 0x2)
        assert_equal(myDisasm.instr.Reserved_.EVEX.P1, 0x5)
        ...
        assert_equal(hex(myDisasm.instr.Instruction.Opcode), '0x28')
        assert_equal(myDisasm.instr.Instruction.Mnemonic, 'vpmuldq ')
        assert_equal(myDisasm.instr.repr, 'vpmuldq ymm0, ymm15, ymmword ptr [rbx+rsi+22h]')

        # EVEX.NDS.512.66.0F38.WIG 28 /r
        # vpmuldq zmm1 {k1}{z}, zmm2, zmm3/m512
        Buffer = '6202054028443322'.decode('hex')
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        assert_equal(myDisasm.instr.Reserved_.EVEX.P0, 0x2)
        assert_equal(myDisasm.instr.Reserved_.EVEX.P1, 0x5)
        ...
        assert_equal(hex(myDisasm.instr.Instruction.Opcode), '0x28')
        assert_equal(myDisasm.instr.Instruction.Mnemonic, 'vpmuldq ')
        assert_equal(myDisasm.instr.repr, 'vpmuldq zmm0, zmm15, zmmword ptr [rbx+rsi+22h]')
