...
class TestSuite:
    def test(self):
        ...
        Buffer = 'c4020129443322'.decode('hex')
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        assert_equal(myDisasm.instr.Instruction.Mnemonic, 'vpcmpeqq ')
        assert_equal(myDisasm.instr.repr, 'vpcmpeqq xmm8, xmm15, xmmword ptr [r11+r14+22h]')

        # VEX.NDS.256.66.0F38.WIG 29 /r
        # vpcmpeqq ymm1, ymm2, ymm3/m256
        Buffer = 'c4020529443322'.decode('hex')
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        assert_equal(myDisasm.instr.Instruction.Mnemonic, 'vpcmpeqq ')
        assert_equal(myDisasm.instr.repr, 'vpcmpeqq ymm8, ymm15, ymmword ptr [r11+r14+22h]')

        # EVEX.NDS.128.66.0F38.WIG 29 /r
        # vpcmpeqq xmm1 {k1}{z}, xmm2, xmm3/m128
        Buffer = '6202050629443322'.decode('hex')
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        ...
        assert_equal(myDisasm.instr.Instruction.Mnemonic, 'vpcmpeqq ')
        assert_equal(myDisasm.instr.repr, 'vpcmpeqq xmm0, xmm15, xmmword ptr [rbx+rsi+22h]')

        # EVEX.NDS.256.66.0F38.WIG 29 /r
        # vpcmpeqq ymm1 {k1}{z}, ymm2, ymm3/m256
        Buffer = '6202052029443322'.decode('hex')
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        assert_equal(myDisasm.instr.Reserved_.EVEX.P0, 0x2)
        assert_equal(myDisasm.instr.Reserved_.EVEX.P1, 0x5)
        ...
        assert_equal(hex(myDisasm.instr.Instruction.Opcode), '0x29')
        assert_equal(myDisasm.instr.Instruction.Mnemonic, 'vpcmpeqq ')
        assert_equal(myDisasm.instr.repr, 'vpcmpeqq ymm0, ymm15, ymmword ptr [rbx+rsi+22h]')

        # EVEX.NDS.512.66.0F38.WIG 29 /r
        # vpcmpeqq zmm1 {k1}{z}, zmm2, zmm3/m512
        Buffer = '6202054029443322'.decode('hex')
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        assert_equal(myDisasm.instr.Reserved_.EVEX.P0, 0x2)
        assert_equal(myDisasm.instr.Reserved_.EVEX.P1, 0x5)
        ...
        assert_equal(hex(myDisasm.instr.Instruction.Opcode), '0x29')
        assert_equal(myDisasm.instr.Instruction.Mnemonic, 'vpcmpeqq ')
        assert_equal(myDisasm.instr.repr, 'vpcmpeqq zmm0, zmm15, zmmword ptr [rbx+rsi+22h]')
