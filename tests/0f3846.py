...
class TestSuite:
    def test(self):
        ...
        myDisasm.read()
        assert_equal(hex(myDisasm.instr.Instruction.Opcode), '0xf3846')
        assert_equal(myDisasm.instr.Instruction.Mnemonic, 'psravd ')
        assert_equal(myDisasm.instr.repr, 'psravd xmm2, xmmword ptr [rax+44332211h]')
        ...
        # EVEX.NDS.256.66.0F38.WIG 46 /r
        # vpsravd ymm1 {k1}{z}, ymm2, ymm3/m256
        Buffer = '6202052046443322'.decode('hex')
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        assert_equal(myDisasm.instr.Reserved_.EVEX.P0, 0x2)
        assert_equal(myDisasm.instr.Reserved_.EVEX.P1, 0x5)
        ...
        # EVEX.NDS.512.66.0F38.WIG 46 /r
        # vpsravd zmm1 {k1}{z}, zmm2, zmm3/m512
        Buffer = '6202054046443322'.decode('hex')
        myDisasm = Disasm(Buffer)
        myDisasm.read()
        assert_equal(myDisasm.instr.Reserved_.EVEX.P0, 0x2)
        assert_equal(myDisasm.instr.Reserved_.EVEX.P1, 0x5)
